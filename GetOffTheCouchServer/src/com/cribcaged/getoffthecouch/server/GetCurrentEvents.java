...
/**
 * Servlet that returns the list of the user's current events
 * @author Yunus Evren
 */
public class GetCurrentEvents extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		result = getCurrentEvents(facebookId);
	}


	/** Gets the current events from the database and prints their details.
	 * @param facebookId - Facebook id of the user
	 * @return true if the current event details are successfully printed
	 */
	private boolean getCurrentEvents(String facebookId) {
		try {
			...
			String checkQuery = "SELECT COUNT(*) FROM event e, user_attends u WHERE u.user_id='" + facebookId + "' AND u.event_id=e.event_id AND e.date>'" + currentDate + "' AND e.confirmed=1 AND e.completed=0";
			...
			String getEventsQuery = "SELECT e.event_id, e.loc_id, l.cat_id, l.loc_name, u.user_name, e.details, e.date, e.time, e.total_score, l.photo_thumb " +
						...
						"AND ua.event_id=e.event_id AND e.date>'" + currentDate + "' AND e.confirmed=1 AND e.completed=0 ORDER BY e.date ASC";
