...
/**
 * Servlet that returns a list of the user's completed events
 * @author Yunus Evren
 */
public class GetCompletedEvents extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		result = getCompletedEvents(facebookId);
	}


	/**
	 * Gets the completed events from the database and prints their details.
	 * @param facebookId - Facebook id of the user
	 * @return true if the events are successfully printed
	 */
	private boolean getCompletedEvents(String facebookId) {
		try {
			Class.forName("com.mysql.jdbc.Driver").newInstance();
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/getoffthecouch", "XXXXXX", "XXXXXX");
			
			Statement checkStmt = conn.createStatement();
			String checkQuery = "SELECT COUNT(*) FROM event e, user_attends u WHERE u.user_id='" + facebookId + "' AND u.event_id=e.event_id AND e.completed=1";
			...
			String getEventsQuery = "SELECT e.event_id, e.loc_id, l.cat_id, l.loc_name, u.user_name, e.details, e.date, e.time, e.total_score, l.photo_thumb " +
						...
						"AND ua.event_id=e.event_id AND e.completed=1 ORDER BY e.date DESC";
